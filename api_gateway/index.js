...
app.get("/users/:userId", async (req, res) => {
  try {
    const user = await usersCircuit.fire(
...
app.delete("/users/:userId", async (req, res) => {
  try {
    const result = await usersCircuit.fire(
      `${USERS_SERVICE_URL}/users/${req.params.userId}`,
      {
        method: "DELETE",
      }
...
app.get("/orders/:orderId", async (req, res) => {
  try {
    const order = await ordersCircuit.fire(
...
app.delete("/orders/:orderId", async (req, res) => {
  try {
    const result = await ordersCircuit.fire(
      `${ORDERS_SERVICE_URL}/orders/${req.params.orderId}`,
      {
        method: "DELETE",
      }
...
app.get("/orders/status", async (req, res) => {
  try {
    const status = await ordersCircuit.fire(
...
app.get("/orders/health", async (req, res) => {
  try {
    const health = await ordersCircuit.fire(
...
app.get("/users/:userId/details", async (req, res) => {
  try {
    ...
    // Get user details
    const userPromise = usersCircuit.fire(
...
app.listen(PORT, () => {
  ...
  usersCircuit.on("halfOpen", () =>
    console.log("Users circuit breaker half-open")
  );
  ...
  ordersCircuit.on("halfOpen", () =>
    console.log("Orders circuit breaker half-open")
  );
});
